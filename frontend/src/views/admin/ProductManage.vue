...
const fetchProducts = async () => {
  loading.value = true
  try {
    const res = await api.get('/admin/products', { params: { page: page.value - 1, size: 10 } })
    if (res.code === 200) {
...
const handleSubmit = async () => {
  ...
  try {
    let res
    if (isEdit.value) {
      res = await api.put(`/admin/products/${editingId.value}`, form)
    } else {
      res = await api.post('/admin/products', form)
    }
    if (res.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确定删除商品"${row.name}"？`, '提示', { type: 'warning' })
    .then(async () => {
      const res = await api.delete(`/admin/products/${row.id}`)
      if (res.code === 200) {
